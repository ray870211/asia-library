...
//
const colWidth = 80, rowHeight = 18;
//
function drawEmployee(doc, employee) {
    ...
    //
    // header
    renderRow(doc, y, columns, (column) => column.header, null, bold, '#fad9cd');
    //
    y += rowHeight;
    //
    // body
    expenses.forEach(item => {
        renderRow(doc, y, columns, (column) => item[column.binding], (column) => column.format);
        y += rowHeight;
    });
    //
    // footer
    let totRow = ['Total', '', tot.hotel, tot.transport, tot.meal, tot.fuel, tot.misc, tot.total];
    renderRow(doc, y, totRow, null, () => 'c', bold, '#fad9cd');
    //
    y += rowHeight;
    //
    doc.y = docY + y * scale;
