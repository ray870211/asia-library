...
function init() {
    ...
    dashboard.forEach(function (item) {
        ...
        tr.appendChild(td);
        let fmt = '<div class="heading">{heading}</div><div class="sub">{sub}</div>';
        ...
        td = document.createElement('td');
        tr.appendChild(td);
        let bg = new gauge.BulletGraph(document.createElement('div'), {
            value: item.actual,
            target: item.target,
            max: item.max,
            bad: item.bad,
        ...
        td = document.createElement('td');
        tr.appendChild(td);
        td.innerHTML = wijmo.format('<span class="max">{max:n0}</span>', item);
    });
}
